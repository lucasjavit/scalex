...
/**
 * Script para popular job_boards e job_board_companies com empresas do We Work Remotely
 * We Work Remotely é um agregador, mas podemos rastrear empresas específicas que postam frequentemente
 * Execute com: npm run seed:weworkremotely-companies
 */
export async function seedWeWorkRemotelyCompanies(dataSource: DataSource) {
  ...
  console.log('🌱 Iniciando seed das empresas We Work Remotely...');

  // 1. Criar/buscar o job board "weworkremotely"
  let weworkremotelyBoard = await jobBoardRepo.findOne({
    where: { slug: 'weworkremotely' },
  });

  if (!weworkremotelyBoard) {
    weworkremotelyBoard = await jobBoardRepo.save({
      slug: 'weworkremotely',
    ...
    console.log('✅ Job board "weworkremotely" criado');
  } else {
    console.log('ℹ️  Job board "weworkremotely" já existe');
  }

  // 2. Lista de empresas que frequentemente postam no We Work Remotely
  // We Work Remotely não tem API por empresa, então usamos busca por empresa no site
  // URL pattern: https://weworkremotely.com/remote-jobs/search?term={company}
  const companies = [
    ...
    // 🚀 Remote-First Companies (frequentemente no We Work Remotely)
    ...
    { slug: 'we-work-remotely', name: 'We Work Remotely', description: 'Remote job board' },
  ];
  ...
  // 3. Para cada empresa, criar/buscar e relacionar com weworkremotely
  for (const companyData of companies) {
    ...
    // We Work Remotely usa busca por termo: https://weworkremotely.com/remote-jobs/search?term={company}
    ...
    if (!existingRelation) {
      await jbcRepo.save({
        jobBoardId: weworkremotelyBoard.id,
        companyId: company.id,
        scraperUrl: `https://weworkremotely.com/remote-jobs/search?term=${encodeURIComponent(companyData.name)}`,
        enabled: true,
        scrapingStatus: null,
        lastScrapedAt: null,
        errorMessage: null,
  ...
  console.log('\n✅ Seed We Work Remotely Companies concluído!');
}
