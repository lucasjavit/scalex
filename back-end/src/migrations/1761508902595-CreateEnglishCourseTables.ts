...
export class CreateEnglishCourseTables1761508902595
  implements MigrationInterface
{
  ...
  private async seedInitialData(queryRunner: QueryRunner): Promise<void> {
    ...
    const stages = await queryRunner.query(
      `SELECT id, title FROM stages ORDER BY order_index`,
    );
    ...
    const stage1Units = await queryRunner.query(
      `SELECT id FROM units WHERE stage_id = '${stages[0].id}' ORDER BY order_index`,
    );

    // Insert Cards for Unit 1
    await queryRunner.query(`
            INSERT INTO cards (id, unit_id, question, answer, example_sentence) VALUES
            (gen_random_uuid(), '${stage1Units[0].id}', 'Como se diz "Olá" em inglês?', 'Hello', 'Hello, how are you?'),
            (gen_random_uuid(), '${stage1Units[0].id}', 'Como se diz "Bom dia" em inglês?', 'Good morning', 'Good morning! Did you sleep well?'),
            (gen_random_uuid(), '${stage1Units[0].id}', 'Como se diz "Prazer em conhecê-lo" em inglês?', 'Nice to meet you', 'Nice to meet you! My name is John.'),
            (gen_random_uuid(), '${stage1Units[0].id}', 'Como se diz "Como vai?" em inglês?', 'How are you?', 'Hi Sarah! How are you doing today?'),
            (gen_random_uuid(), '${stage1Units[0].id}', 'Como se diz "Tchau" em inglês?', 'Goodbye', 'Goodbye! See you tomorrow.');
        `);

    // Insert Cards for Unit 2
    await queryRunner.query(`
            INSERT INTO cards (id, unit_id, question, answer, example_sentence) VALUES
            (gen_random_uuid(), '${stage1Units[1].id}', 'Como se diz o número "1" em inglês?', 'One', 'I have one apple.'),
            (gen_random_uuid(), '${stage1Units[1].id}', 'Como se diz o número "10" em inglês?', 'Ten', 'There are ten students in the class.'),
            (gen_random_uuid(), '${stage1Units[1].id}', 'Como se diz o número "20" em inglês?', 'Twenty', 'I am twenty years old.'),
            (gen_random_uuid(), '${stage1Units[1].id}', 'Como se diz o número "100" em inglês?', 'One hundred', 'This book has one hundred pages.');
        `);

    // Insert Cards for Unit 3
    await queryRunner.query(`
            INSERT INTO cards (id, unit_id, question, answer, example_sentence) VALUES
            (gen_random_uuid(), '${stage1Units[2].id}', 'Como se diz "Por favor" em inglês?', 'Please', 'Can you help me, please?'),
            (gen_random_uuid(), '${stage1Units[2].id}', 'Como se diz "Obrigado" em inglês?', 'Thank you', 'Thank you for your help!'),
            (gen_random_uuid(), '${stage1Units[2].id}', 'Como se diz "De nada" em inglês?', 'You''re welcome', 'You''re welcome! Anytime.'),
            (gen_random_uuid(), '${stage1Units[2].id}', 'Como se diz "Desculpe" em inglês?', 'Sorry / Excuse me', 'Sorry, I didn''t mean to interrupt.'),
            (gen_random_uuid(), '${stage1Units[2].id}', 'Como se diz "Com licença" em inglês?', 'Excuse me', 'Excuse me, where is the bathroom?');
        `);
    ...
    const stage2Units = await queryRunner.query(
      `SELECT id FROM units WHERE stage_id = '${stages[1].id}' ORDER BY order_index`,
    );

    // Insert Cards for Unit 4
    await queryRunner.query(`
            INSERT INTO cards (id, unit_id, question, answer, example_sentence) VALUES
            (gen_random_uuid(), '${stage2Units[0].id}', 'Complete: I ____ (visit) Paris three times.', 'have visited', 'I have visited Paris three times.'),
            (gen_random_uuid(), '${stage2Units[0].id}', 'Complete: She ____ (not finish) her homework yet.', 'hasn''t finished', 'She hasn''t finished her homework yet.'),
            (gen_random_uuid(), '${stage2Units[0].id}', 'Complete: ____ you ever ____ (eat) sushi?', 'Have / eaten', 'Have you ever eaten sushi?');
        `);

    // Insert Cards for Unit 5
    await queryRunner.query(`
            INSERT INTO cards (id, unit_id, question, answer, example_sentence) VALUES
            (gen_random_uuid(), '${stage2Units[1].id}', 'O que significa "give up"?', 'Desistir', 'Don''t give up on your dreams!'),
            (gen_random_uuid(), '${stage2Units[1].id}', 'O que significa "look after"?', 'Cuidar de', 'Can you look after my cat this weekend?'),
            (gen_random_uuid(), '${stage2Units[1].id}', 'O que significa "turn on"?', 'Ligar (aparelho)', 'Please turn on the lights.'),
            (gen_random_uuid(), '${stage2Units[1].id}', 'O que significa "turn off"?', 'Desligar (aparelho)', 'Don''t forget to turn off the TV.');
        `);
    ...
    const stage3Units = await queryRunner.query(
      `SELECT id FROM units WHERE stage_id = '${stages[2].id}' ORDER BY order_index`,
    );

    // Insert Cards for Unit 6
    await queryRunner.query(`
            INSERT INTO cards (id, unit_id, question, answer, example_sentence) VALUES
            (gen_random_uuid(), '${stage3Units[0].id}', 'Como se diz "reunião" em inglês?', 'Meeting', 'We have a meeting at 3 PM.'),
            (gen_random_uuid(), '${stage3Units[0].id}', 'Como se diz "prazo" em inglês?', 'Deadline', 'The deadline for this project is Friday.'),
            (gen_random_uuid(), '${stage3Units[0].id}', 'Como se diz "acordo" em inglês?', 'Agreement / Deal', 'We reached an agreement with the client.');
        `);
