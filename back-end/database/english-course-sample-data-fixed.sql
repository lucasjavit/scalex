-- ============================================
-- ENGLISH COURSE - COMPLETE SAMPLE DATA
-- Insert all lessons and questions (basic + additional)
-- ============================================

-- ============================================
-- BASIC LESSONS (originally in init.sql)
-- ============================================
...
-- Verify that all lessons were inserted correctly
DO $$
DECLARE
    lesson_count INTEGER;
    question_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO lesson_count FROM english_lessons;
    SELECT COUNT(*) INTO question_count FROM english_questions;
    
    RAISE NOTICE 'Successfully inserted % lessons with % questions!', lesson_count, question_count;
    RAISE NOTICE 'Lesson distribution:';
    RAISE NOTICE '- Beginner: % lessons', (SELECT COUNT(*) FROM english_lessons WHERE level = 'beginner');
    RAISE NOTICE '- Elementary: % lessons', (SELECT COUNT(*) FROM english_lessons WHERE level = 'elementary');
    RAISE NOTICE '- Intermediate: % lessons', (SELECT COUNT(*) FROM english_lessons WHERE level = 'intermediate');
    RAISE NOTICE '- Advanced: % lessons', (SELECT COUNT(*) FROM english_lessons WHERE level = 'advanced');
    RAISE NOTICE 'English Course is ready for use!';
END $$;
